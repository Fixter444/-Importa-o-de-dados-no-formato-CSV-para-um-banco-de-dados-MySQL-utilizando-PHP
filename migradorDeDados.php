...
try {
			...
			while (($data = fgetcsv($handle, 1000, ';')) !== false)
				{
					...
					while ($row = $stmtCon->fetch(PDO::FETCH_ASSOC)) 
					{
							...
							echo $id . ' - ' . 'Nome: ' . $data[1] . ' ID Cliente: ' . $row['id'] . " " . " (". $parametro .")" . PHP_EOL;
							$cliente_id = $row['id'];
							$idcli = $cliente_id;
					...
					else 
					{
						...
						echo $id . ' - ' . 'Nome: ' . $data[1] . ' ID Financeiros: ' . $idcli . " " . " (". $parametro2 .")" . PHP_EOL;
						echo "##################################################################################################." . PHP_EOL;
						// Escreve a instrução INSERT no arquivo de texto
						fwrite($file, $id . ' - ' . 'Nome: ' . $data[1] . ' ID Financeiros: ' . $idcli . " " . " (". $parametro2 .")" . PHP_EOL);
						readline();
						continue;
					} 
					...
					while ($row = $stmtCon->fetch(PDO::FETCH_ASSOC)) 
					{
						echo $id . ' - ' . 'Nome: ' . $data[1] . ' ID Contrato: ' . $row['id'] . " " ."(". $parametro .")" . PHP_EOL;
						$idContrato = $row['id'];
					...
					catch (Exception $e) 
					{
						...
						echo 'Ocorreu um erro ao inserir os dados: ' . $e->getMessage() . PHP_EOL;
						echo "##########################################################" . PHP_EOL;
						echo "Falha ao inserir Financeiro: ". $id . ' - ' . $data[0] . PHP_EOL;
						echo "Precione ENTER para continuar!" . PHP_EOL;
						echo "##########################################################" . PHP_EOL;
	...
	echo "########################################################" . PHP_EOL;
	echo "       Importação do CSV concluída com sucesso!" . PHP_EOL;
	echo "########################################################" . PHP_EOL;
